...
@LYCIA.on_message(filters.command("Shukurenai"))
async def Lycia(_, message):
    ...
    m = await message.reply_text("Shukurenai Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Deepak&ownername=@deepakjack007&user=1"
        )
        chatbot = L["message"]
